...
import struct
from scipy.signal import butter, lfilter, lfilter_zi, filtfilt, decimate
import sys
import time
import copy
import pandas as pd
...
def dblog(msg):
    # wrapper for sending debug log messages. 
    # The idea is that this will be overridden with more sophisticated logging
...
#   next. Sweeps are therefore variable in length.
    def __init__(self, circuitFile=None, ephysFileName=None):
        self.running=False
        self.sweepLen=1.0 
        self.sweeps=[]
    ...
    def trigger(self):
        ...
        self.timestampBuffer.append(now-self.time0)
    ...
    def processNewSweep(self,sweepStart,sweepEnd):
        ...
        if nSamplesExpected > nSamplesGot:
            dblog('')
            dblog('*** ERROR *** ERROR *** ERROR *** ERROR ')
            dblog('*** processNewSweep expected: {} got:{}. '.format(nSamplesExpected,nSamplesGot))
            dblog('')
        ...
        newSweep.saveSweep(self.ephysFileHandle)
        self.sweeps.append(newSweep)
        oldLen=len(self.sigBuf)
        self.sigBuf=self.sigBuf[nSamplesExpected:]
        self.sweepEnd=sweepEnd
        dblog('--- Added {} samples to new sweep with time stamp {:.5f}. {} sweeps in RAM'.format(nSamplesExpected, tStamp, len(self.sweeps)))
        dblog('')
        dblog('--- sigBuf length went *** from {:,} to {:,} ***.'.format(oldLen, len(self.sigBuf)))
        dblog('')
    ...
    def start(self):
        ...
        self.sweepEnd=0    
        self.totalUploaded=0
        self.timestampBuffer=[time.time()-self.time0]
        ...
        self.openEphysFileHandle()
        dblog('recorder started.')
        # sys.stdout.flush()
    ...
    def stop(self):
        ...
        dblog('recorder stopped.')
        # self.saveAcquiredSweeps()
        # sys.stdout.flush()
        config.uploadInProgress=False
        self.closeEphysFileHandle()

    def plotSweep(self):
        try:
            if self.sweepToPlot is None:
                # dblog('plotSweep(): nothing to plot!')
                return
            if self.figure is None:
                # allocating new figure
                self.figure=plt.figure()
            # dblog('switching interactive mode off '); sys.stdout.flush()
            # plt.ioff()
            ...
            dblog('NOTE: plotSweep() failed !', str(e))
        # then delete the sweep    
        self.sweepToPlot=None
        # gc.collect()
        # dblog('switching interactive mode back on'); sys.stdout.flush()
        # plt.ion()

    def openEphysFileHandle(self):  
        # make sure ephysFileHandle is open
        if self.ephysFileHandle is None:
            if not self.ephysFileName is None:
                dblog('opening data file '+self.ephysFileName)
                self.ephysFileHandle = open(self.ephysFileName,'ab')
                # XXX Todo: consder putting error handling in here
            
    def closeEphysFileHandle(self):  
    ...
    def saveAcquiredSweeps(self):
        ...
        # ***Deprecated*** as we move to multiprocessing
            ...
            #  -> open file handle and try again
                ...
                if  hasattr(self,'ephysFile'):
                    if self.ephysFile is IOBase:
                        ...
                        self.ephysFileHandle = open(self.ephysFileName,'ab')
        if not self.ephysFileHandle is None:
            dblog('Saving {} sweep(s)'.format(len(self.sweeps)))
...
class TDT_RZ2_Cont_Recorder(Virtual_Cont_Recorder):
    ...
    def __init__(self, circuitFile='MC-continuous-1x32atlasOmniToZIF.rcx', ephysFileName=None):
        ...
        super().__init__(ephysFileName=ephysFileName)
        ...
        self.zBus = ZBUSx()
        dblog('Connecting to RZ2 ephys recorder with circuit '+circuitFile)
        ...
        self.uploadBufferTimer=None
        # self.processBufferTimer=None
        self.uploadBufferInterval=0.3
        
    def getCircuitTriggerIndex(self):
        return int(self.RP.GetTagVal('TriggerIdx'))
    ...
    def getLatestSweepStartAndEnd(self):                   
        ...
        self.latestTriggerIdx=self.getCircuitTriggerIndex()
        ...
        if self.numTriggersRead > self.numTriggersProcessed:
            ...
            # # dblog('')
            # dblog('   ====> Last recording trigger index is ',self.latestTriggerIdx)
            # dblog('   ====> Triggers processed is ',self.numTriggersProcessed)
            # dblog('   Trigger samples: ',self.triggerSamples)
            dblog('   ====> processing sweep Nr {} from sample {} to {}'.format(self.numTriggersProcessed,fromIdx,toIdx))
    ...
    def uploadData(self):
        ...
        try:
            ...
            self.sigBuf=np.append(self.sigBuf,sig,axis=0)
            # dblog('sig buf dtypes :', sig.dtype, self.sigBuf.dtype )
            self.totalUploaded+=nextChunkSize
            # dblog('    uploaded {} samples so far'.format(self.totalUploaded)); sys.stdout.flush()
            # check if a sweep has ended.
            ...
            while newSweepCompleted: # this tells us that the new sweepEnd is beyond the last remembered sweepEnd, so a new sweep is finished                
                ...
                # dblog('time stamp buffer now:', self.timestampBuffer)
                # dblog('Allocating new sweep from samples {} to {}'.format(sweepStart,sweepEnd))
    ...
    def trigger(self):
        dblog('TDT_RZ2_Cont_Recorder.trigger() triggering on zBus.')
    ...
    def start(self):
        ...
        # self.uploadBufferTimer=RepeatedTimer(self.uploadBufferInterval,self.uploadData)
        self.uploadBufferTimer=RepeatedTimer(min(self.uploadBufferInterval,self.sweepLen/2),self.uploadData)
...
class TDT_fixedSweep_RZ2_Cont_Recorder(TDT_RZ2_TimerSave_Cont_Recorder):
    ...
    def uploadData(self):
        ...
        try:
            ...
            dblog('Last upload {:3.2f} seconds ago'.format(timeSinceLastUpload))
            if timeSinceLastUpload > self.maxUploadInterval:
                dblog()
                dblog('***')
                dblog('ERROR: data upload rate not keeping up with buffer!')
                dblog('***')
                dblog()
                self.stop()
            ...
            if self.sweepsRecorded<self.sweepsToRecord: 
                ...
                dblog('Recorded sweep {} out of {}. (ADidx is {})'.format(self.sweepsRecorded,self.sweepsToRecord, self.currIdx))
    ...
    def start(self, ISIsamples, sweepsToRecord):
        ...
        if isinstance(self.ephysFileName, str):
            ...
            self.ephysFileHandle = None
        dblog('Starting fixed sweep size recorder to record {} sweeps of {} samples ({} s) each.'.format(\
...
#%%
class RecorderBackgroundProcess(mp.Process):
    """A wrapper allowing recorders to be run in the background on a 
       separate core. Recorded sweeps are pushed onto a queue for online
       display.
    """
    def __init__(self, recorderClass=Virtual_Cont_Recorder, circuitFile=None, ephysFileName=None):
        super().__init__()
        self.recorderClass=recorderClass
        self.circuitFile=circuitFile
        self.ephysFileName=ephysFileName
        self.swpQueue = mp.Queue()
        self.msgQueue = mp.Queue()

    def send(self, item):
        """Puts the item on the msgQueue 
        """
    ...
    def dispatch(self, msg):
        if msg=='start':
            dblog('Starting recorder in background process.')
            self.recorder.start()
            return
        if msg=='stop':
            dblog('Stopping recorder in background process.')
            self.recorder.stop()
            return
        if msg=='trigger':
            dblog('Tiggering recorder in background process.')
            self.recorder.trigger()
            return
        
    def run(self):
        self.recorder=self.recorderClass(circuitFile=self.circuitFile, ephysFileName=self.ephysFileName)
        while True:
            while len(self.recorder.sweeps)>0:
                # first we make sure that acquired sweeps are saved,
                # then we pass them on to the multiprocessing queue
                sweepToProcess=self.recorder.sweeps.pop(0)
                # if not self.recorder.ephysFileHandle is None:
                #     sweepToProcess.saveSweep(self.recorder.ephysFileHandle)
                self.swpQueue.put(sweepToProcess)
...
class MCsweep:    
    ...
    def LFPfilterCoeffs(self, lowpass=300, highpass=30):
...
#        result.signal=insig[padLen:-padLen,:]
    ...
    def calcAMUA(self,coefs=[], padLen=300, downsample=2):
        # make a AMUA copy of the sweep 
        # Calculates analog multiunit activity (AMUA) measure as in periodic gerbil paper.
        # See http://journal.frontiersin.org/article/10.3389/fncir.2015.00037/full
        result=MCsweep(self.sampleRate/downsample) # decimation will make AMUA have half orig sample rate
...
#        result.signal=insig[padLen:-padLen,:]
    ...
    def plotSig(self, nCol=8, pltHandle=plt, channels=[], lineObjects=[]): # plot a multichannel signal
        if self.signal is None:
            dblog('Signal empty. Nothing to plot.')
            return
...
def poolSweeps(swp):
    ...
    for ss in range(len(swp)):
        ...
        #dblog('sweep {}: length {}'.format(ss, swp[ss].signal.shape[0]))
...
def readEphysStimTable(fname):
    ...
    try:
        ...
        dblog('warning, no stim file found for '+fname)
        stimPar=None
    return stimPar
...
def readMchanSignalFromFile(fileObject):
    try:
        ...
        #dblog('Finished reading sweeps with message: ',str(e))
...
def interpolateAwayPulseArtefacts(s,nSigma=3):
    # this function attempts to find very sharp peaks (probably due to electrical artefacts)
    # by looking for very sharp curvature
    d=np.abs(np.diff(np.diff(s,axis=0),axis=0))
    #%%
    thresh=np.std(d,axis=0)*nSigma
    # thresh=np.percentile(d,pcCutOff,axis=0)
    ii=0
    #%%
    plt.clf()
    plt.plot(d[:,ii])
    plt.plot([0,d.shape[0]],[thresh[ii],thresh[ii]])
    plt.xlim([0,1000])
    ii+=1
    #%
    artPos=np.where(d[:,ii]>thresh[ii])[0]
    nonDup=np.where(np.diff(np.flip(artPos))<-2)[0]
    artPos=artPos[nonDup]
    plt.plot(artPos,np.ones(len(artPos))*thresh[ii]*2,'*')
